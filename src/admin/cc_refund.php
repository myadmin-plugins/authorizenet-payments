		...
		while ($db->next_record(MYSQL_ASSOC)) {
			...
			if ($dbR->num_rows() == 0) {
				...
				if ($do == 'void') {
					...
					$checkbox .= '<div style="display: inline-block;width: 130px; font-weight: bold; text-align: right; padding-right: 15px;">$' .$db->Record['invoices_amount'].'</div></div>';
					$checkbox .= '</label><br>';
				} else {
					$checkbox .= '<input type="checkbox" name="refund_amount_opt[]" value="'.$db->Record['invoices_service'].'_'.$db->Record['invoices_id'].'_'.$db->Record['invoices_amount'].'" onclick="return update_partial_payment();" checked>&nbsp;<label for="" style="text-transform: capitalize; margin-bottom: 15px; font-weight: normal;"> ';
					$checkbox .= '<div><div style="display: inline-block;width: 130px;">'.$db->Record['invoices_module'].'</div>';
					$checkbox .= '<div style="display: inline-block;width: 130px;">'.$db->Record['invoices_service'].'</div></div>';
				...
				$checkbox .= '<input type="checkbox" name="refund_amount_opt[]" value="'.$db->Record['invoices_service'].'_'.$db->Record['invoices_id'].'_0" disabled="disabled">&nbsp;<label for="" style="text-transform: capitalize; margin-bottom: 15px; font-weight: normal;"> ';
				$checkbox .= '<div><div style="display: inline-block;width: 130px;">'.$db->Record['invoices_module'].'</div>';
				$checkbox .= '<div style="display: inline-block;width: 130px;">'.$db->Record['invoices_service'].'</div></div>';
